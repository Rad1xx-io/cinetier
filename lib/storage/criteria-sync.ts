...
async function findRatingId(
  ...
  // RankedTitle.animeSource, RankedTitle.mobileGameSource) — without it, two
  // entries sharing a number would make `.maybeSingle()` below throw instead
  // of finding either one's breakdown.
  const { data } = await supabase
    ...
    .eq("source", toSourceColumn(mediaType, gameSource, animeSource, mobileGameSource))
    .maybeSingle();
...
export async function pushCriteria(
  ...
  const ratingId = await findRatingId(userId, tmdbId, mediaType, gameSource, animeSource, mobileGameSource);
