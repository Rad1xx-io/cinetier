...
describe("BattleResults — the score", () => {
  it("shows the match percentage and a verdict", () => {
    ...
    expect(screen.getByText("Много общего")).toBeDefined();
  });

  it("says how much of the pool was actually compared", () => {
    renderResults();

    expect(screen.getByText("Сравнили 3 из 3 позиций.")).toBeDefined();
  });
  ...
  it("refuses to show 0% when there was nothing to compare", () => {
    ...
    expect(screen.getByText("Сравнивать нечего")).toBeDefined();
  ...
  it("still shows 0% when the two genuinely disagreed on everything", () => {
    ...
    expect(screen.getByText("Полярные вкусы")).toBeDefined();
    expect(screen.queryByText("Сравнивать нечего")).toBeNull();
  });
});
...
describe("BattleResults — comparison lists", () => {
  it("lists agreements and disagreements with both sides' tiers", () => {
    ...
    expect(screen.getByText("Полное согласие")).toBeDefined();
    expect(screen.getByText("Главные споры")).toBeDefined();
    ...
    const agreements = section("Полное согласие");
    const disagreements = section("Главные споры");
    ...
    expect(screen.getAllByText("Автор").length).toBeGreaterThan(0);
  ...
  it("hides a section that has nothing in it", () => {
    ...
    expect(screen.queryByText("Главные споры")).toBeNull();
    expect(screen.getByText("Полное согласие")).toBeDefined();
  });
...
describe("BattleResults — sharing", () => {
  ...
  it("offers a way to start a battle of your own", () => {
    ...
    const cta = screen.getByRole("link", { name: /Создать свой батл/ });
